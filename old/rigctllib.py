...
class RigCtl(object):
    ...
    def __init__(self, target):
        ...
        try:
            self.con = telnetlib.Telnet(target["hostname"], target["port"], RIG_TIMEOUT)
        except socket.timeout:

            logger.error(
                "Time out while connecting to {}:{}".format(
            ...
            logger.exception(
                "Connection refused on {}:{}".format(target["hostname"], target["port"])
            )
            raise
    ...
    def _request(self, request, target=None):
        ...
        for i in range(tries):
            try:
                self.con.write((f"{request}\n").encode("utf-8"))
                response = self.con.read_lazy().decode("utf-8").strip()
    ...
    def set_frequency(self, frequency, target=None):
        ...
        try:
            ...
            logger.error("Frequency value must be a float, " "got {}".format(frequency))
            raise
    ...
    def set_mode(self, mode, bw=0, target=None):
        """Wrapper around _request. It configures the command for setting
        the mode.
        ...
        if not isinstance(mode, str) or mode not in ALLOWED_RIGCTL_MODES:
            ...
            logger.error(
                "Frequency mode must be a string in {}, "
                "got {}".format(ALLOWED_RIGCTL_MODES, mode)
            )
            raise ValueError
        return self._request(f"M {mode} {bw}", target)
    ...
    def get_mode(self, target=None):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting radio mode, "
                "got {}".format(output)
            )
            raise ValueError
        return output
    ...
    def get_rfpower(self):
        ...
        output = self._request("l RFPOWER")
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting radio "
                "signal level, got {}".format(output)
    ...
    def set_vfo(self, vfo):
        ...
        if vfo not in ALLOWED_VFO_COMMANDS:
            logger.error(
                "VFO value must be a string inclueded in {}, "
                "got {}".format(ALLOWED_VFO_COMMANDS, vfo)
            )
            raise ValueError

        return self._request(f"V {vfo}")
    ...
    def get_vfo(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting VFO, " "got {}".format(output)
            )
            raise ValueError
    ...
    def set_rit(self, rit):
        ...
        if not isinstance(rit, int):
            logger.error("RIT value must be an int, " "got {}".format(type(rit)))
            raise ValueError
    ...
    def get_rit(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting RIT, "
    ...
    def set_xit(self, xit):
        ...
        if not isinstance(xit, str):
            logger.error("XIT value must be a string, " "got {}".format(type(xit)))
            raise ValueError
    ...
    def get_xit(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting XIT, "
                "got {}".format(type(output))
            )
            raise ValueError
    ...
    def set_split_freq(self, split_freq):
        ...
        if not isinstance(split_freq, int):
            logger.error(
                "XIT value must be an integer, " "got {}".format(type(split_freq))
            )
            raise ValueError
    ...
    def get_split_freq(self):
        ...
        if not isinstance(output, int):
            logger.error(
                "Expected int while getting split_frequency, "
                "got {}".format(type(output))
            )
            raise ValueError
    ...
    def get_split_mode(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected string while getting split_frequency_mode, "
                "got {}".format(type(output))
            )
            raise ValueError
    ...
    def set_func(self, func):
        ...
        if func not in ALLOWED_FUNC_COMMANDS:
            logger.error(
                "func value must be a string inclueded in {}, "
    ...
    def get_func(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting func, " "got {}".format(output)
            )
            raise ValueError
        return output
    ...
    def set_parm(self, parm):
        ...
        if parm not in ALLOWED_PARM_COMMANDS:
            logger.error(
                "parm value must be a string inclueded in {}, "
    ...
    def get_parm(self):
        ...
        if not isinstance(output, str):
            logger.error(
                "Expected unicode string while getting parm, " "got {}".format(output)
            )
            raise ValueError
    ...
    def set_antenna(self, antenna):
        ...
        if not isinstance(antenna, int):
            logger.error("antenna value must be an int, " "got {}".format(antenna))
            raise ValueError
    ...
    def get_antenna(self):
        ...
        if not isinstance(output, int):
            logger.error(
                "Expected integer while getting radio antenna, " "got {}".format(output)
            )
            raise ValueError
    ...
    def rig_reset(self, reset_signal):
        ...
        if reset_signal not in RESET_CMD_DICT.keys():
            logger.error(
                "Reset_signal must be one of " "{}.".format(RESET_CMD_DICT.keys())
            )
            raise ValueError
